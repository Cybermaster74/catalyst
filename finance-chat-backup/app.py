...
def analyze():
    ...
    if request.method == 'POST':
        try:
            ...
            date_str = request.form.get('analysis_date', datetime.now().strftime('%Y-%m-%d'))
            ...
            try:
                ...
                # Standardize at midnight
                analysis_date = datetime.combine(analysis_date.date(), datetime.min.time())
                print(f"Analysis date set to: {analysis_date.strftime('%Y-%m-%d')}")
            except ValueError as e:
                # Default to current date if invalid
                print(f"Error parsing date '{date_str}': {e}")
                today = datetime.now().date()
                analysis_date = datetime.combine(today, datetime.min.time())
            ...
            for symbol in symbols:
                try:
                    ...
                    result = analyze_stock(symbol, analysis_date, take_profit=take_profit)
...
def analyze_stock(symbol, analysis_date, take_profit=0.08):
    """
    ...
    try:
        # Ensure analysis_date is timezone naive and standardized
        if hasattr(analysis_date, 'tzinfo') and analysis_date.tzinfo is not None:
        ...
        # Ensure all pandas Timestamps are timezone naive 
        # Convert historical_data index to naive timestamps if needed
        ...
        # Make sure analysis_timestamp is timezone naive
        if hasattr(analysis_timestamp, 'tz') and analysis_timestamp.tz is not None:
            analysis_timestamp = analysis_timestamp.tz_localize(None)
            
        closest_date = min(available_dates, key=lambda x: abs(x - analysis_timestamp))
        
        if closest_date != analysis_timestamp:
        ...
        # Add technical indicators to results - making sure we extract the indicators dictionary properly
        if hasattr(technical_indicators, 'attrs') and 'indicators' in technical_indicators.attrs:
            results["technical"] = technical_indicators.attrs['indicators']
        else:
            # If indicators aren't in attrs, create a basic dictionary with essential price info
            latest_row = technical_indicators.iloc[-1]
            results["technical"] = {
                'price': float(latest_row['Close']),
                'sma_20': float(latest_row['SMA_20']) if 'SMA_20' in latest_row else None,
                'sma_50': float(latest_row['SMA_50']) if 'SMA_50' in latest_row else None,
                'sma_200': float(latest_row['SMA_200']) if 'SMA_200' in latest_row else None,
                'rsi': float(latest_row['RSI']) if 'RSI' in latest_row else None,
                'volume': float(latest_row['Volume']) if 'Volume' in latest_row else None
            }
        
        if market_data is not None:
            market_indicators = calculate_indicators(market_data)
            if market_indicators is not None and not market_indicators.empty:
                # Similar to technical indicators, extract from attrs if available
                if hasattr(market_indicators, 'attrs') and 'indicators' in market_indicators.attrs:
                    results["market_technical"] = market_indicators.attrs['indicators']
                else:
                    # If indicators aren't in attrs, create a basic dictionary with essential price info
                    latest_row = market_indicators.iloc[-1]
                    results["market_technical"] = {
                        'price': float(latest_row['Close']),
                        'sma_20': float(latest_row['SMA_20']) if 'SMA_20' in latest_row else None,
                        'sma_50': float(latest_row['SMA_50']) if 'SMA_50' in latest_row else None,
                        'sma_200': float(latest_row['SMA_200']) if 'SMA_200' in latest_row else None,
        ...
        backtest_results = backtest_strategy(historical_data, analysis_date, 0, take_profit)
        if backtest_results:
...
def allocate_investment(total_amount, results, market_condition=None):
    """
    Allocate investment amount to stocks with BUY recommendations, prioritizing those with good entry timing.
    ...
    # First pass: collect all stocks with BUY recommendations, prioritizing those with good entry timing
    for result in results:
        if 'error' not in result and 'recommendation' in result:
            rec = result.get('recommendation', {}).get('recommendation', '')
            if 'BUY' in rec:
                # Check entry timing - prioritize stocks with good entry but don't require it
                entry_quality = result.get('entry_quality', False) or result.get('entry_timing', {}).get('good_entry', False)
                
                # Calculate a bonus for good entry timing
                timing_bonus = 0.3 if entry_quality else 0
                score = result.get('recommendation', {}).get('score', 0)
                signal_strength = result.get('recommendation', {}).get('signal_strength', 0.5) + timing_bonus
                buy_candidates.append((result, signal_strength))
    
    # If no buy candidates found, return empty allocations
...
def analyze_market_condition():
    ...
    try:
        ...
        if spy_data is None:
            print("Failed to calculate market indicators")
            return 'neutral'
            
        # Verify required columns exist
        required_columns = ['Close', 'SMA_50', 'SMA_200', 'RSI', 'MACD', 'MACD_Signal']
        ...
        try:
            ...
            print(f"Error accessing market data columns: {e}")
            return 'neutral'
    except Exception as e:
