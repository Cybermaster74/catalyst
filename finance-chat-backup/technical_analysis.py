...
def calculate_indicators(df):
    ...
    try:
        ...
        # Add backward compatibility for code that might use old column names
        df['SMA20'] = df['SMA_20']
        df['SMA50'] = df['SMA_50']
        ...
        df['MACD_Signal'] = df['Signal_Line']  # Add standardized column name
        ...
        # Also add indicators as dictionary properties in the dataframe metadata
        if len(df) > 0:
            # Get latest values
            latest = df.iloc[-1]
            ...
            prev_signal = prev_day['MACD_Signal'] if 'MACD_Signal' in prev_day else prev_day['Signal_Line']
            macd_crossover = (macd > signal and prev_macd <= prev_signal)
            macd_crossunder = (macd < signal and prev_macd >= prev_signal)
            
            # Store indicators in dataframe metadata
            df.attrs['indicators'] = {
                'price': price,
                'sma_20': sma20,
                'sma_50': sma50,
                'sma_200': sma200,
                'sma20': sma20,  # Keep backward compatibility
                'sma50': sma50,  # Keep backward compatibility
                'sma200': sma200,  # Keep backward compatibility
                ...
                'short_term_trend': 'up' if sma20_trend > 0 else 'down',
                'medium_term_trend': 'up' if sma50_trend > 0 else 'down',
                'long_term_trend': 'up' if sma200_trend > 0 else 'down',
                'trend_score': (sma20_trend * 0.5) + (sma50_trend * 0.3) + (sma200_trend * 0.2)
            }
...
def analyze_indicators(df, date, technical_indicators=None):
    ...
    :param technical_indicators: Optional dictionary of pre-calculated technical indicators or DataFrame with indicators
    ...
    if technical_indicators is not None:
        ...
        # Check if technical_indicators is a DataFrame with attrs, a DataFrame without attrs, or a dictionary
        if hasattr(technical_indicators, 'attrs') and 'indicators' in technical_indicators.attrs:
            # Use the indicators dictionary from attrs
            indicators_dict = technical_indicators.attrs['indicators']
        elif isinstance(technical_indicators, pd.DataFrame):
            # Extract indicators from the DataFrame directly
            if date in technical_indicators.index:
                row = technical_indicators.loc[date]
            else:
                closest_date = technical_indicators.index[technical_indicators.index <= pd.Timestamp(date)].max()
                if pd.isnull(closest_date):
                    return None
                row = technical_indicators.loc[closest_date]
            
            # Create indicators dictionary from the row
            indicators_dict = {
                'price': float(row['Close']),
                'sma20': float(row['SMA_20']) if 'SMA_20' in row else None,
                'sma50': float(row['SMA_50']) if 'SMA_50' in row else None, 
                'sma200': float(row['SMA_200']) if 'SMA_200' in row else None,
                'price_above_sma20': float(row['Close']) > float(row['SMA_20']) if 'SMA_20' in row else None,
                'price_above_sma50': float(row['Close']) > float(row['SMA_50']) if 'SMA_50' in row else None,
                'price_above_sma200': float(row['Close']) > float(row['SMA_200']) if 'SMA_200' in row else None,
                ...
                'volume_ratio': float(row['Volume_Ratio']) if 'Volume_Ratio' in row else None
            }
        else:
        ...
        # Trend signals
        signals['trend'] = {}
        signals['trend']['sma_20_above_50'] = 1 if indicators_dict.get('sma20', 0) > indicators_dict.get('sma50', 0) else -1
        signals['trend']['sma_50_above_200'] = 1 if indicators_dict.get('sma50', 0) > indicators_dict.get('sma200', 0) else -1
        signals['trend']['price_above_sma_20'] = 1 if indicators_dict.get('price_above_sma20', False) else -1
        signals['trend']['price_above_sma_50'] = 1 if indicators_dict.get('price_above_sma50', False) else -1
        signals['trend']['price_above_sma_200'] = 1 if indicators_dict.get('price_above_sma200', False) else -1
        
        # Momentum signals
        signals['momentum'] = {}
        signals['momentum']['macd_above_signal'] = 1 if indicators_dict.get('macd', 0) > indicators_dict.get('macd_signal', 0) else -1
        signals['momentum']['macd_positive'] = 1 if indicators_dict.get('macd', 0) > 0 else -1
        signals['momentum']['macd_crossover'] = 2 if indicators_dict.get('macd_crossover', False) else 0
        signals['momentum']['macd_crossunder'] = -2 if indicators_dict.get('macd_crossunder', False) else 0
        signals['momentum']['rsi_overbought'] = -2 if indicators_dict.get('rsi', 0) > 70 else 0
        signals['momentum']['rsi_oversold'] = 2 if indicators_dict.get('rsi', 0) < 30 else 0
        signals['momentum']['rsi_trending_up'] = 1 if indicators_dict.get('rsi', 0) > 50 else -1
        
        # Volume signals
        signals['volume'] = {}
        signals['volume']['above_average'] = 1 if indicators_dict.get('volume_ratio', 0) > 1.2 else -1
        ...
        signals['trend_strength'] = {
            'adx_strong': indicators_dict.get('adx', 0) > 25,
            'adx_very_strong': indicators_dict.get('adx', 0) > 40
        }
